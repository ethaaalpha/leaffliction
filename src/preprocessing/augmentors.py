...
class Augmentators:
    ...
    def blur(self) -> Im.Image:
        return self._image.filter(
            ImageFilter.GaussianBlur(random.uniform(0.5, 4)))

    def contrast(self) -> Im.Image:
        return ImageEnhance.Contrast(self._image).enhance(
            random.uniform(1.5, 2.0))

    def brightness(self) -> Im.Image:
        return ImageEnhance.Brightness(self._image).enhance(
